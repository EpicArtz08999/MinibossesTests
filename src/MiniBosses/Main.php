...
class Main extends PluginBase implements Listener {
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $args) {
		if(!isset($args[0])) {
			...
			if(!($sender instanceof Player)) {
				...
				$sender->sendMessage(TF::RED . "Usage: /minibosses create network-id name");
			}
		} elseif($args[0] === "spawn") {
			if(count($args) >= 2) {
				array_shift($args);
				...
				if($this->data->get($name, null) !== null) {
					...
					if($ret === true) {
						$sender->sendMessage(TF::GREEN . "Successfully spawned MiniBoss: $name");
					} else {
						$sender->sendMessage(TF::RED . "Error spawning MiniBoss: $name : $ret");
					}
				} else {
					$sender->sendMessage(TF::RED . "That MiniBoss does not exist!");
				}
			} else {
	...
	public function spawnBoss(string $name = "Boss") {
		...
		$nbt = new CompoundTag("", [
			...
			"lightningDamage" => new IntTag("lightningDamage", $lightningDamage)
		]);
	...
	public function respawn($name, $time) {
		if($this->data->get($name)) {
			$this->getServer()->getScheduler()->scheduleDelayedTask(new RespawnTask($this, $name), $time);
		}
	}
}
